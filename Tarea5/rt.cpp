...
class Abedo : public material {
    public:
        ...
        virtual bool Rebota(
            const Ray &wi, Color& atenuacion,Ray &wo) const override {
...
Color shade(const Ray &r,int prof) { //Agregamos la profundidad para hacer una funcion recursiva, esto nos permite lanzar un segundo rayo desde
	...
	Ray rebota(x,re);
    if (!obj.m->Rebota(r, attenuation,rebota)) //Si el rayo es un emisor Rebota te regresa Falso, por lo que ste if se vuelve True
